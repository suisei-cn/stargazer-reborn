    ...
    Hasher: BuildHasher + Clone,
{
    ...
    /// # Edge Case
    ///
    /// If the ring had keys but no nodes, you should insert all keys into this new node.
    /// The `Migrated` struct won't handle this for you.
    ...
    /// i.e. if there's no node in the ring or the key doesn't exist, returns `None`.
